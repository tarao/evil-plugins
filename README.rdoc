...
Evil state (such as `normal`, `insert`, etc.) and the value of the
associative list must be a string of a color (such as "SteelBlue4",
"#575735", etc.).
...
list must be a symbol of visual state type (such as `normal`, `line`
and `block`) and the value of the associative list must be a string of
indicator.
...
A patch for `hexl-mode`, a mode for editing binary files in hex dump
format.
